...
func (a *App) SyncEmailVector(gmailID string, bodyText string) error {
	...
	if resp == nil || resp.Embedding == nil {
		return fmt.Errorf("Ollamaから空のレスポンスが返りました")
	}
	...
	for i := 0; i < 3; i++ {
		_, err = a.db.Exec("INSERT OR REPLACE INTO email_vectors (id, content, vector) VALUES (?, ?, ?)", gmailID, cleanText, vectorBytes)
		if err == nil {
			fmt.Printf("⏳ DB.書き込み成功しました。\n")
			return nil // 成功！
		}
		// DBがロックされていたら、少し（0.5秒〜）待って再挑戦
		fmt.Printf("⏳ DBロック中、リトライします... (%d/3)\n", i+1)
